...
const calc = (totalCost = 0, initalFee = 50000, creditTerm = 1) => {
  let ammountLoan = totalCost - initalFee;
  let interestRate = currentPercent;
  let ammountMonth = creditTerm * 12;

  let MonthlyPayment = (ammountLoan + (((ammountLoan * interestRate) / 100) / 12 * ammountMonth) / ammountMonth);
  ...
  if (MonthlyPaymentRound < 0) {
    ...
    totalCredit.innerHTML = `${ammountLoan}₴`;
